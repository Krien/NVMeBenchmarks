...
void interleaved_reset_append_test(uint64_t zone_cnt, uint64_t zone_cap,
                                   uint64_t zone_size, char *buf,
                                   DeviceTarget *target,
                                   t_spdk_nvme_qpair **qpairs) {
  uint64_t zones_to_reset = zone_cnt / 2;
  fill_zones_append(0, zones_to_reset, zone_cap, zone_size, buf, target,
                    qpairs);
...
int main(int argc, char **argv) {
  ...
  clear_device(target.ns, qpairs[0]);
  interleaved_reset_append_test(info.zone_cnt, info.zone_cap, info.zone_size,
                                buf, &target, qpairs);
