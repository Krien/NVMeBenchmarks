...
# Out names
SPDK_READ_OP: str = "read"
IO_URING_READ_MQ_OPTION: str = "readmq"
IO_URING_READ_NONE_OPTION: str = "read"
...
def main(
    ...
    job_defaults = [
        JobOption(JobWorkload.SEQ_READ),
        DirectOption(True),
    ...
    # Fill device
    fill_path = BenchPath(
        IOEngine.IO_URING, model, lbaf, "prefill_grid_read", 1, 1, "128K"
    ).AbsPathOut()
    fio.run_job(
    ...
    for (qd, bs, concurrent_zones) in [
        ...
        if zns:
            ...
            operation = IO_URING_READ_MQ_OPTION
        else:
            sjob.add_options([SchedulerOption(Scheduler.NONE)])
            operation = IO_URING_READ_NONE_OPTION
        job.add_job(sjob)
